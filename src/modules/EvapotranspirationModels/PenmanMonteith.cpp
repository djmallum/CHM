...
{
	static const double mm_per_m = 1000;
	...
	double radiation = delta(var.t) * Q;  //Units: W/m^2 * kPa/K
	
	// Units: W/m^2 * kPa/K
	double mass = AirDensity(var.t,var.vapour_pressure,var.P_atm) * heat_capacity_air * 
        (var.saturated_vapour_pressure - var.vapour_pressure)/ aero_resistance;
	...
	out.ET *= mm_per_m * s_per_time_step; // Converts from m/s to mm/(time step)
