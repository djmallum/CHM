...
public:
    ...
    [[nodiscard]] explicit Params(const DomainParameters* domain_parameters) : domain_parameters(domain_parameters) {}
    [[nodiscard]] Metre z_0() const {return Metre{vegetation_height/7.6};}
    [[nodiscard]] Metre d() const {return Metre{vegetation_height*0.67};}
    [[nodiscard]] bool has_vegetation() const {return vegetation_height>0.0;}
    [[nodiscard]] Percent porosity() const;
private:
...
SecondsPerMetre get_stomatal_resistance(const Input& input,const Params& parameters)
{
    ...
    if (input.has_vegetation())
    {

        double LAI = input.vegetation_height()/2.0*parameters.leaf_area_index_max; //TODO ad hoc for test
        stomatal_res_min.value = parameters.stomatal_resistance_min * leaf_area_index_max / LAI;
    ...
    double p = parameter.air_entry_tension() * pow(input.porosity() / (input.soil_storage/input.soil_depth()/1000.0 + parameters.wilt_point()), parameters.pore_size_dist());
