...
{
    std::unique_ptr<I_Darcy_Vels> Vels = std::make_unique<Darcy_Vels>(DTO);
...
{ 
    double unit_changer = DTO.get_dt(DTO) * 1000.0; // seconds/step * mm/m
...
void Darcy_Vels::calculate()
{
    ...
    if( !(DTO.soil_rechr_storage == 0.0 && DTO.soil_storage_max == 0.0) )
    {
        if (DTO.swe > 0.0)
            set_snow();
        else
...
{
    return DTO.Ksaturated_organic * pow(DTO.detention_storage/DTO.detention_max,3.0 + 2.0/DTO.pore_size_dist_organic) * tan(DTO.local_slope);
};
