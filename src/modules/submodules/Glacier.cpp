...
namespace Glacier
{
	...
	std::optional<Units::Milimetres> LayeredFirn::convert_to_ice()
    {
        ...
        assert(p->densify_version == DensifyVersion::Linear && "convert_to_ice overload called with non-matching p::densify_version");
        ...
        {
            layer.densifyLinear(p->small_increment,p->big_increment,p->critical_density);
        }

        if (layers.front().density() > p->firn_to_ice_density)
        {
    ...
    std::optional<Units::Milimetres> LayeredFirn::convert_to_ice(const Units::Kelvin T,
            const Units::Milimetres mean_annual_accumulation_rate)
    {
        std::optional<Units::Milimetres> result;

        assert(p->densify_version == DensifyVersion::HerronLangway && "convert_to_ice overload called with non-matching p::densify_version");
        ...
        {
            it->densifyHerronLangway(T,depth,mean_annual_accumulation_rate.value,p->critical_density);
            depth += it->height().value;
        }
