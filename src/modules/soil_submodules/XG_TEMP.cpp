...
void XG_algorithm::execute_impl(Data& d)
{
    if (d.is_newday())
    {
        auto& s = get_freezethaw_state();

        auto phase = s.history.phase();

        s.thermal.set_conductivities(phase);

        s.moisture.set_freezethaw_ratios(phase);
