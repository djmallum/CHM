...
TEST(ParamsKatabaticTest,ParamsSetByPhysConst)
{
    ...
    EXPECT_EQ(p.molecular_wt_ratio,PhysConst::em());
...
TEST_F(KatabaticHeatTest,AnalyticlLatentHeat)
{
    ...
    const Units::Pa vapour_pressure_deficit{1e3};
    const Units::Pa air_pressure{1e5};
    const auto expected = p.molecular_wt_ratio * air_density.value * PhysConst::Lv(glacier_temperature) * coefficient.value * vapour_pressure_deficit.value / air_pressure.value;

    auto result = latent_heat(p,coefficient,vapour_pressure_deficit,glacier_temperature,air_density,air_pressure);
