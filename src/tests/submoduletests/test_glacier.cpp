...
[[maybe_unused]] static inline Params test_default_params()
{
    ...
    p.small_increment = Units::DensitySI{25.0};
    p.big_increment = Units::DensitySI{50.0};
    p.critical_density = Units::DensitySI{550.0};
	p.firn_to_ice_density = Units::DensitySI{830.0};
...
TEST_F(LayeredFirnTest,ConvertToIceLinear)
{
    ...
    const auto old_layers = firn->get_layers();
    ...
    {
        auto density_new = new_layers[i].density();
        auto density_old = layers[i+1].density();
        if (density_old <= p.firn_to_ice_density)
            EXPECT_EQ(density_new.value,density_old.value + p.small_increment.value);
        else
            EXPECT_EQ(density_new.value,density_old.value + p.big_increment.value);
    };

};

TEST_F(LayeredFirnTest,ConvertToIceHerronLangway)
{
    std::deque<Layer> layers = old_layers();
    ...
    const Units::Kelvin glacier_temp{270.0};
    const Units::Milimetres mean_annual_accumulation_rate{0.1};

    auto result = firn->convert_to_ice(glacier_temp,mean_annual_accumulation_rate);
    ...
    auto depth = 0.0;
    auto it_old = old_layers.rbegin();
    auto it_new = new_layers.rbegin();
    while (it_new != new_layers.rend())
    {
        auto density_new = it_new->density();
        auto density_old = it_old->density();
        auto height_new = it_new->height();
        auto height_old = it_old->height();

        auto depth = std::accumulate(new_layers.rbegin(),it_new,0.0,
                [](double acc,const Layer& layer)
                {
                    return acc + layer.height().value;
                });

        depth += height_old.value;

        auto expected_density = Densification::HerronLangway(depth,
                density_old.value,glacier_temp.value,mean_annual_accumulation_rate.value,
                p.critical_density.value);

        EXPECT_EQ(density_new.value,expected_density);

        if (density_new.value == density_old.value)
            EXPECT_EQ(height_new.value,height_old.value);
        else
            EXPECT_GT(height_old.value,height_new.value);
...
protected:
    ...
    void set_firn()
    {
		...
		{
			EXPECT_LT(layer.density().value,p.firn_to_ice_density.value);
		};
...
protected:
	...
	LayeredFirn old_firn()
	{
        ...
        auto firn_to_ice_density = p.firn_to_ice_density.value;
		std::array<Local,6> local{{
			{{4.0},{250.0}},{{3.2},{420.0}},{{2.5},{750.0}},
			{{1.2},{firn_to_ice_density + 20.0}},
			{{0.6},{firn_to_ice_density + 45.0}},
			{{0.3},{firn_to_ice_density + 100.0}}
		}};
		...
		{
			if (layer.density().value > p.firn_to_ice_density.value)
				++count_over_critical;
		}
...
TEST_F(YearlyUpdaterTest,FreshFirnUnchangedIceAndFirn)
{
	...
	EXPECT_DOUBLE_EQ(init_firn.water_equivalent().value,
...
TEST_F(YearlyUpdaterTest,ManyIceUpdates)
{
	...
	{	
		if (layers.front().water_equivalent().value < p.firn_to_ice_density.value)
			break;
        ++iter_tracker;
...
TEST_F(ModelTest, UpdateNowTrue_DenseFirnConvertsToIce)
{
    ...
    Layer::Height h{1.0};
    Layer::Density rho{p.firn_to_ice_density.value + 10.0};  // above threshold
...
{
    Layer::Height h{1.0};
    Layer::Density rho{p.firn_to_ice_density.value + 10.0};
    Layer dense_layer(h, rho);
